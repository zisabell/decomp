...
lml_2 <- lml_2 %>% mutate(litter_mass_loss_final = ifelse(litter_mass_loss_final > 1,
                                                         1, litter_mass_loss_final))
lml_2$litter_mass_loss_final_no_0_1 <- pmin(pmax(lml_2$litter_mass_loss_final, 
                                                 0.0001), 0.9999)
lml_2$logit_mass_loss <- log(lml_2$litter_mass_loss_final_no_0_1 /
                               (1 - lml_2$litter_mass_loss_final_no_0_1))

#different models
three_way_gauss <- lmer(litter_mass_loss_final ~ landuse * litter_sp * mesh_size 
                               + (1|location), data=lml_2, na.action="na.omit")
...
three_way_beta <- glmmTMB(litter_mass_loss_final_no_0_1 ~ landuse * litter_sp *
                          ...
                          dispformula = ~ litter_sp, data=lml_2, na.action="na.omit")
three_way_zi <- glmmTMB(litter_mass_loss_final_no_0_1 ~ landuse * litter_sp * mesh_size 
                     + (1|location), beta_family(), ziformula = ~1, 
                     data=lml_2, na.action="na.omit")

BIC(three_way_gauss,three_way_gauss_disp,three_way_gauss_arcsin, three_way_beta, three_way_zi) #beta is best
simres <- simulateResiduals(three_way_beta)
plot(simres) #severe issues
# anova(three_way_beta)
emm <- emmeans(three_way_beta, ~ landuse | litter_sp | mesh_size, type="response")
...
two_way_lu_lq <- lme4::lmer(litter_mass_loss_final ~ landuse * litter_sp + (1|location), data=lml_2, na.action = "na.omit")
emm <- emmeans(two_way_lu_lq, ~ landuse | litter_sp, type="response")
(contrasts <- contrast(emm, "pairwise", by="litter_sp") %>%
     rbind())
...
(contrasts <- contrast(emm, "pairwise", by="mesh_size") %>% rbind())
