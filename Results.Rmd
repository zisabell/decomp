...
lml_wide <- lml_wide %>% mutate(alder_faunal = Coarse_Alder-Fine_Alder,
                    hazel_faunal = Coarse_Hazel-Fine_Hazel,
                    chestnut_faunal = Coarse_Chestnut-Fine_Chestnut)
...
lml_wide <- lml_wide %>% filter(!grepl("VB002|VB116|VB118",vb_code))

lml_wide_long <- lml_wide %>% pivot_longer(cols = c("alder_faunal","hazel_faunal","chestnut_faunal"), values_to="faunal_effect")

lml_wide %>% ggplot(aes(x=earthworms_2022)) + 
  ...
  ylab("Faunal effect (%)") + 
  xlab("Earthworm abundance") 
  
ggsave("faunal_effect_ew.png", height=8, width=8)
