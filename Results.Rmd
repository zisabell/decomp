...
```
## boxplots AMF, oribatid and EW

``` {r boxplots}
mt_4 %>% ggplot(aes(x=landuse, y=AMF_NLFA, fill=landuse)) + 
  ...
  labs(x="Landuse", y="Estimation of AMF biomass (ug/gr soil)",  fill="Landuse")

ggsave("AMF.svg", width=10, height = 8)

mt_4 %>% ggplot(aes(x=landuse, y=Mites_oribatid_kg, fill=landuse)) + 
  geom_boxplot(outlier.shape = NA, alpha=0.8, show.legend = FALSE) +
  scale_fill_manual(values=wes_palette("Royal1"), labels=c("Food forest","Grassland")) + 
  geom_point(position = position_jitterdodge(0.4), 
             pch=16, color="black",
  ...
  theme_bw(base_size = 24) +
  scale_x_discrete(labels=c("Food forest", "Grassland")) +
  labs(x="Landuse", y="Oribatid mite abundance (#/kg soil)",  fill="Landuse")

ggsave("Oribatid.svg", width=10, height = 8)

mt_4 %>% ggplot(aes(x=landuse, y=biomass_ew_total, fill=landuse)) + 
  geom_boxplot(outlier.shape = NA, alpha=0.8, show.legend = FALSE) +
  scale_fill_manual(values=wes_palette("Royal1"), labels=c("Food forest","Grassland")) + 
  geom_point(position = position_jitterdodge(0.4), 
             pch=16, color="black",
  ...
  theme_bw(base_size = 24) +
  scale_x_discrete(labels=c("Food forest", "Grassland")) +
  labs(x="Landuse", y="Earthworm biomass (g/m2)",  fill="Landuse")

ggsave("EW.svg", width=10, height = 8)
```
...
emm <- emmeans(three_way_gauss_disp, ~ landuse * litter_sp * mesh_size, type="response")
emm %>% tidy()
joint_tests(three_way_gauss_disp) %>% flextable()
(contrasts <- contrast(emm, "pairwise", simple="each", combine = TRUE, adjust="Sidak"))
# confint(contrasts)# confint(contrasts)tidy()
...
image <- lml_3 %>%
  ...
  scale_color_manual(values=c("#E1BE6A","#40B0A6"),
                    labels=c("Food forest","Grassland")) + 
  # geom_point(aes(color=landuse),position = position_jitterdodge(0.4), 
  #            pch=16, show.legend = FALSE) +
  geom_beeswarm(aes(color = landuse), dodge.width = 1, cex = 2.5, show.legend = FALSE) +
  geom_point(
    data = three_way_cld,
  ...
  geom_text(data = three_way_cld, aes(x = landuse, y = upper.CL, label = .group)) +
  facet_grid(litter_sp ~ mesh_size) +
  theme_bw(base_size = 20) +
  scale_x_discrete(labels=c("Food forest", "Grassland")) +
  labs(y="Litter mass loss", x="Land use", fill="Landuse")
image
ggsave(file="3x3plot_new_letters.png", plot=image, width=10, height=10)
```

```{r three way NEW LMLLLLLL}
#take NAs out
lml_3 <- filter(lml_2, !is.na(litter_mass_loss_final_3))
#transform data necessary for different models
# lml_3 <- lml_3 %>% mutate(litter_mass_loss_final = ifelse(litter_mass_loss_final > 1,
#                                                          1, litter_mass_loss_final))
# lml_3$litter_mass_loss_final_no_0_1 <- pmin(pmax(lml_3$litter_mass_loss_final, 
...
#                                + (1|location), family=gaussian(), 
#                                data=lml_3, na.action="na.omit") #TERRIBLE
three_way_gauss_disp <- glmmTMB(litter_mass_loss_final_3 ~ landuse * litter_sp * 
                                  mesh_size + (1|location), 
                               family=gaussian, dispformula = ~ litter_sp,
                        data=lml_3, na.action="na.omit")
# three_way_gauss_arcsin <- lmer(asin(sqrt(litter_mass_loss_final)) ~ 
...
emm <- emmeans(three_way_gauss_disp, ~ landuse * litter_sp * mesh_size, type="response")
emm_tidy <- emm %>% tidy()
emm_lu <- emmeans(three_way_gauss_disp, pairwise ~ landuse | litter_sp | mesh_size, type="response")
emm_lu
joint_tests(three_way_gauss_disp) %>% flextable()
...
# Plot the outcomes
image <- lml_3 %>% group_by(mesh_size, litter_sp) %>%
  ggplot(aes(x=landuse, y=litter_mass_loss_final_3, fill=landuse, color=landuse)) +
  geom_boxplot(outlier.shape = NA,color = NA, alpha=0.4, show.legend = FALSE) +
  ...
  # geom_point(position = position_jitterdodge(0.4), 
  #            pch=16, color="black",show.legend = FALSE) + 
  geom_beeswarm(aes(color = landuse), dodge.width = 1, cex = 3, pch=19, show.legend = FALSE) +
  geom_point(
    data = three_way_cld,
    aes(y = emmean, x = landuse),
    size = 3,
...
image
ggsave(file="3x3plot_new_letters.png", plot=image, width=10, height=10)
ggsave(file="LML_final.svg", width=10, height=10)
```


```{r two way tests + visualisation}
two_way_lu_lq <- lmer(litter_mass_loss_final ~ landuse * litter_sp + (1|location),
                         data=lml_2, na.action = "na.omit")
simres <- simulateResiduals(two_way_lu_lq)
plot(simres)
emm <- emmeans(two_way_lu_lq, ~ landuse * litter_sp, type="response")
(contrasts <- contrast(emm, "pairwise", simple="each") %>%
     rbind()) %>% tidy()

lu_lq <- lml_2 %>% group_by(litter_sp) %>%
  ggplot(aes(x=landuse, y=litter_mass_loss_final, fill=landuse)) +
  geom_boxplot(outlier.shape = NA, alpha=0.8, show.legend = FALSE) +
  ...
  geom_point(position = position_jitterdodge(0.4), 
             pch=16, color="black",show.legend = FALSE) +
  # geom_text(data = threeway_cld, aes(x = landuse, y = upper.CL, label = .group)) +
  facet_grid(~ litter_sp) +
  theme_bw(base_size = 20) +
  scale_x_discrete(labels=c("Food forest", "Grassland")) +
  labs(y="Litter mass loss (%)", x="Land use", fill="Landuse")
lu_lq

two_way_lu_mesh <- lme4::lmer(litter_mass_loss_final ~ landuse * mesh_size + (1|location), data=lml_2, na.action = "na.omit")
emm <- emmeans(two_way_lu_mesh, ~ landuse * mesh_size, type="response")
(contrasts <- contrast(emm, "pairwise", simple="each") %>% rbind())

lu_mesh <- lml_2 %>% group_by(mesh_size) %>%
  ggplot(aes(x=landuse, y=litter_mass_loss_final, fill=landuse)) +
  geom_boxplot(outlier.shape = NA, alpha=0.8, show.legend = FALSE) +
  scale_fill_manual(labels=c("Food forest","Grassland")) + 
  scale_fill_brewer(palette="Paired") +
  ...
  geom_point(position = position_jitterdodge(0.4), 
             pch=16, color="black",show.legend = FALSE) +
  # geom_text(data = threeway_cld, aes(x = landuse, y = upper.CL, label = .group)) +
  facet_grid(~ mesh_size) +
  theme_bw(base_size = 20) +
  scale_x_discrete(labels=c("Food forest", "Grassland")) +
  labs(y="Litter mass loss (%)", x="Land use", fill="Landuse")
lu_mesh

two_way_lp_mesh <- lme4::lmer(litter_mass_loss_final ~ litter_sp * mesh_size + (1|location), data=lml_2, na.action = "na.omit")
emm <- emmeans(two_way_lp_mesh, ~ litter_sp * mesh_size, type="response")
(contrasts <- contrast(emm, "pairwise",simple="each") %>% rbind())

lp_mesh <- lml_2 %>% group_by(litter_sp) %>%
  ggplot(aes(x=litter_sp, y=litter_mass_loss_final, fill=litter_sp)) +
  geom_boxplot(outlier.shape = NA, alpha=0.8, show.legend = FALSE) +
  # scale_fill_manual(labels=c("Food forest","Grassland")) + 
  # scale_fill_brewer(palette="Paired") +
  # scale_fill_manual(values=c("#33A02B","#FF7F00")) +
  ...
  theme_bw(base_size = 20) +
  # scale_x_discrete(labels=c("Food forest", "Grassland")) +
  labs(y="Litter mass loss (%)", x="Litter species", fill="Landuse")
lp_mesh
```

```{r one way}
one_way_lu <- lme4::lmer(litter_mass_loss_final ~ landuse + (1|location), data=lml_2, na.action = "na.omit")
emm <- emmeans(one_way_lu, ~ landuse, type="response")
(contrasts <- contrast(emm, "pairwise", simple="each") %>% rbind())

one_way_litter <- lme4::lmer(litter_mass_loss_final ~ litter_sp + (1|location), data=lml_2, na.action = "na.omit")
emm <- emmeans(one_way_litter, ~ litter_sp, type="response")
(contrasts <- contrast(emm, "pairwise", simple="each") %>% rbind())

one_way_mesh <- lme4::lmer(litter_mass_loss_final ~ mesh_size + (1|location), data=lml_2, na.action = "na.omit")
emm <- emmeans(one_way_mesh, ~ mesh_size, type="response")
(contrasts <- contrast(emm, "pairwise", simple="each") %>% rbind())
```
