...
mt_2 <- mt_2 %>% rowwise() %>% mutate(earthworms_2022 = sum(c_across(contains("Abundance")))*10)
...
mt_4 <- cbind(mt_4, biomass_ew_total = 
                rowSums(mt_2[c('Biomass_anecic','Biomass_endogeic','Biomass_epigeic','Biomass_juvenile')])*10)
...
vars <- mt_4 %>%
  dplyr::select(ph_kcl, som_percentage, b.d., ec, moisture_percentage, NO2_NO3_mg_kg, NH4_mg_kg,
            ...
            Bacteria_total, bact_total, Microbial, Actinobacteria_total, Fungi_total, Total_plfa, AMF_NLFA,
            C_total, N_total, P_total, bact_total, CN) %>%
  names() %>%
  set_names()
...
models <- list()
for (i in c('ph_kcl','som_percentage','b.d.','moisture_percentage', 'ec', 'NO2_NO3_mg_kg','NH4_mg_kg',
            'Total_P_mg_kg','Olsen_P_mg_kg','C_total','N_total','P_total',
            'earthworms_2022','biomass_ew_total',
            'Springtails_nr_kg','Mites_oribatid_kg','Mites_other_kg','nr_nematodes_gr_dry',
            'Bacteria_total','Microbial','Actinobacteria_total','Fungi_total','Total_plfa','AMF_NLFA', 'bact_total', 
            'CN')) {
...
``` {R data check ec, fig.dim=c(3,5)}
mt_4 %>% group_by(landuse) %>% summarize(mean=mean(ec),sd=sd(ec),var=var(ec))
variances_loc_lu$ec
mt_4 %>% group_by(landuse) %>% ggplot(aes(x=landuse, y=ec, color=landuse)) + geom_boxplot() + theme(legend.position='none')
plot(models$ec)
qqPlot(residuals(models$ec))
anova(models$ec)
shapiro.test(residuals(models$ec))

var_log_ec <- leveneTest(log(ec) ~ landuse * location, data=mt_4) #unequal variances fixed!
mod_log_ec <- lmerTest::lmer(log(ec+0.001) ~ landuse + (1|location), data=mt_4,
                  na.action = "na.omit")
qqPlot(residuals(mod_log_ec))
shapiro.test(residuals(mod_log_ec))
anova(mod_log_ec)
...
anova(models$ph_kcl)
# ranef(models$ph_kcl)
simres <- simulateResiduals(models$ph_kcl)
plot(simres)

# # exp(pH)
# var_exp_ph <- leveneTest(exp(ph_kcl) ~ landuse * location, data=mt_4) #Still unequal variance!!
# mod_exp_ph <- lmerTest::lmer(exp(ph_kcl) ~ landuse + (1|location), data=mt_4,
#                   na.action = "na.omit")
# qqPlot(residuals(mod_exp_ph))
# shapiro.test(residuals(mod_exp_ph)) # normality fixed
# anova(mod_exp_ph)
# simres <- simulateResiduals(mod_exp_ph)
# plot(simres) # KS significant
...
Results untransformed data with linear mixed model:  
- p-value = `r anova(models$ph_kcl)$'Pr(>F)'`  
- F-value = `r anova(models$ph_kcl)$'F value'`  
...
mt_4 %>% group_by(landuse) %>% ggplot(aes(x=landuse, y=som_percentage, color=landuse)) + geom_boxplot() + geom_point() + theme(legend.position='none')
...
simres <- simulateResiduals(models$som_percentage)
plot(simres)
```
...
simres <- simulateResiduals(models$b.d.)
plot(simres) #looks fine
```
...
simres <- simulateResiduals(models$moisture_percentage)
plot(simres) #looks fine
```
...
simres <- simulateResiduals(models$NO2_NO3_mg_kg)
plot(simres) #looks fine

# # Log transformation
# var_log_N <- leveneTest(log(NO2_NO3_mg_kg) ~ landuse * location, data=mt_4) #UNEQUAL VARIANCES FIXED!
# mod_log_N <- lmerTest::lmer(log(NO2_NO3_mg_kg+0.001) ~ landuse + (1|location), data=mt_4,
#                   na.action = "na.omit")
# # anova(mod_log_N)
...
Conclusion: no transformation with linear mixed model
...
anova(models$Total_P_mg_kg)

simres <- simulateResiduals(models$Total_P_mg_kg)
plot(simres) #looks fine

# Log transformation FIXED!!
var_log_P <- leveneTest(log(Total_P_mg_kg) ~ landuse * location, data=mt_4)
mod_log_P <- lmerTest::lmer(log(Total_P_mg_kg+0.001) ~ landuse + (1|location), data=mt_4,
                  na.action = "na.omit")
qqPlot(residuals(mod_log_P))
shapiro.test(residuals(mod_log_P))
anova(mod_log_P)

simres <- simulateResiduals(mod_log_P)
plot(simres) #looks fine
```
...
simres <- simulateResiduals(models$Olsen_P_mg_kg)
plot(simres) #looks fine

# Log transformation FIXED!!
var_log_P_2 <- leveneTest(log(Olsen_P_mg_kg) ~ landuse * location, data=mt_4) #unequal variances fixed!
mod_log_P_2 <- lmerTest::lmer(log(Olsen_P_mg_kg+0.001) ~ landuse + (1|location), data=mt_4,
                  na.action = "na.omit")
qqPlot(residuals(mod_log_P_2))
shapiro.test(residuals(mod_log_P_2))
anova(mod_log_P_2)

simres <- simulateResiduals(mod_log_P_2)
plot(simres) #looks fine
```
...
mod_log_N_3 <- lmerTest::lmer(log(N_total+0.001) ~ landuse + (1|location), data=mt_4,
                  na.action = "na.omit")
...
mod_log_C <- lmerTest::lmer(log(C_total+0.001) ~ landuse + (1|location), data=mt_4,
                  na.action = "na.omit")
...
``` {R data check CN, fig.dim=c(3,5)}
mt_4 %>% group_by(landuse, location) %>% summarize(mean=mean(CN),sd=sd(CN),var=var(CN))
variances_loc_lu$CN #no equal variance
mt_4 %>% group_by(landuse) %>% ggplot(aes(x=landuse, y=CN, color=landuse)) + geom_boxplot() + theme(legend.position='none')
plot(models$CN)
qqPlot(residuals(models$CN))
shapiro.test(residuals(models$CN)) #normal

# Log transformation FIXED!!
var_log_CN <- leveneTest(log(CN) ~ landuse * location, data=mt_4) #fixed!
mod_log_CN <- lmerTest::lmer(log(CN+0.001) ~ landuse + (1|location), data=mt_4,
                  na.action = "na.omit")
qqPlot(residuals(mod_log_CN))
shapiro.test(residuals(mod_log_CN))
anova(mod_log_CN)
emm <- emmeans(mod_log_CN, ~landuse)
contrasts <- contrast(regrid(emm, "log"), method= "pairwise", type="response")
contrasts
```
**C total conclusion:**  
...
simres <- simulateResiduals(models$Mites_oribatid_kg)
plot(simres) #looks okayish
...
mod_sqrt_ori <- lmer(sqrt(Mites_oribatid_kg) ~ landuse + (1|location), data=mt_4,
                  na.action = "na.omit")
qqPlot(residuals(mod_sqrt_ori))
shapiro.test(residuals(mod_sqrt_ori))
anova(mod_sqrt_ori)
simres <- simulateResiduals(mod_sqrt_ori)
plot(simres) #looks betterrr
```

**Oribatid mites conclusion**  
Equal variances, non-normal residuals from shapiro-wilk (p=`r round(shapiro.test(residuals(models$Mites_oribatid_kg))$p.value, digits=3)`). But QQ-plot looks good (1 outlier). Transformation with sqrt helps.
...
simres <- simulateResiduals(models$Mites_other_kg)
plot(simres) #looks okayish
...
var_sqrt_mit <-leveneTest(sqrt(Mites_other_kg)~ landuse * location, data=mt_4)
mod_sqrt_mit <- lmer(sqrt(Mites_other_kg) ~ landuse + (1|location), data=mt_4,
                  na.action = "na.omit")
shapiro.test(residuals(mod_sqrt_mit))
anova(mod_sqrt_mit)

simres <- simulateResiduals(mod_sqrt_mit)
plot(simres) #looks okayish
```
...
``` {R data check bact_total, fig.dim=c(3,5)}
mt_4 %>% group_by(landuse, location) %>% summarize(mean=mean(bact_total),sd=sd(bact_total),var=var(bact_total))
variances_loc_lu$bact_total
mt_4 %>% group_by(landuse) %>% ggplot(aes(x=landuse, y=bact_total, color=landuse)) + geom_boxplot() + theme(legend.position='none')
plot(models$bact_total)
qqPlot(residuals(models$bact_total))
shapiro.test(residuals(models$bact_total))
anova(models$bact_total)

simres <- simulateResiduals(models$bact_total)
plot(simres) #looks okayish
```

**Total bacteria conclusion:**  
Unequal variances (p=`r variances_loc_lu$Bacteria_total$"Pr(>F)"[1]`), normal residuals with shapiro. QQ-plot looks good. Use non-transformed data.

- p-value = `r anova(models$bact_total)$'Pr(>F)'`  
- F-value = `r anova(models$bact_total)$'F value'` 
...
simres <- simulateResiduals(models$Fungi_total)
plot(simres) #looks okay

# # Log transformation
# var_log_fun <- leveneTest(log(Fungi_total) ~ landuse * location, data=mt_4) #equal variances!!
# mod_log_fun <- lmer(log(Fungi_total+0.0001) ~ landuse + (1|location), data=mt_4,
#                   na.action = "na.omit")
...
Equal variances, non-normal residuals with shapiro (p=`r round(shapiro.test(residuals(models$Fungi_total))$p.value, digits=3)`). Conclusion: use non-transformed data in linear mixed model.  

- p-value = `r anova(models$Fungi_total)$'Pr(>F)'`  
- F-value = `r anova(models$Fungi_total)$'F value'`  
...
simres <- simulateResiduals(models$AMF_NLFA)
plot(simres) #looks okay
...
simres <- simulateResiduals(mod_log_amf)
plot(simres) #looks much better!
