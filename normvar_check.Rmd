...
mod_log_ec <- lmerTest::lmer(log(ec+0.001) ~ landuse + (1|location/landuse), data=mt_4,
                  na.action = "na.omit")
...
#GLMM linear with log link
ph_glmm_log <- lme4::glmer(ph_kcl ~ landuse + (1|location/landuse), data=mt_4, na.action = "na.omit", family=gaussian(link="log"))
Anova(ph_glmm_log, type=3)
summary(ph_glmm_log)

summary(glht(ph_glmm_log, mcp(landuse = "Tukey")), test=adjusted("fdr"))
```
**pH conclusion:**  
...
som_glmm <- lme4::glmer(som_percentage ~ landuse + (1|location/landuse), data=mt_4, na.action = "na.omit", family=gaussian(link="identity"))
anova(som_glmm)
summary(som_glmm)
...
mod_sqrt_mit <- lmer(sqrt(Mites_other_kg) ~ landuse + (1|location/landuse), data=mt_4,
                  na.action = "na.omit")
...
# Log transformation
var_log_bact <- leveneTest(log(bact_total) ~ landuse * location, data=mt_4) #equal variances!!
mod_log_bact <- lmer(log(bact_total+0.0001) ~ landuse + (1|location/landuse), data=mt_4,
                  na.action = "na.omit")
